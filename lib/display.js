	...
	generateScript = function generateScript(image, hash, callback){
		try{
			...
			script.push('<div id="node-cap_div">');
				script.push('<script type="text/javascript" src="/node-captcha/scripts.js"></script>');
				if(helpers.getConfig("useStyles", true)){
//					script.push('<link rel="stylesheet" type="text/css" href="/node-captcha/styles.css" />');
					script.push('<style type="text/css">');
					script.push('@import url("/node-captcha/styles.css");');
					if(helpers.getConfig("displayRefreshImage", true)){
						script.push('#node-cap_inputrefresh {'+
							' width: 26px;' +
							' height: 26px;' +
							' font-size: 13px;' +
							...
							' cursor: hand;' +
							' border: 0;' + 
							' background-image: url("/node-captcha/refreshimage/'+helpers.getConfig("refreshImageColor", "#2C2")+'"); '+
						'}');
						script.push('#node-cap_inputtext {'+
							' clear: left;' +
							' display: block;' +
							' float: left;' +
							' font-size: 17px;' +
							...
							' border-radius: 5px; ' +
						'}');
						script.push('#node-cap_div input[type="text"] {'+
							' margin: 2px;' +
